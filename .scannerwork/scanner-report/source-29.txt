...
function readFile() {
  ...
  var readata = fs.readFileSync('/home/admin238/Desktop/funduKeepNotes/BackEnd/test/test.json')
              ...
              chai.request(server).post('/login').send(data.login).end((err, res) => {
                if (err) {
                  ...
                  describe('Forgot Password page', function () {
                    it('status ', function (done) {
                      chai.request(server).post('/forgot').send(data.forgotPassword).end((err, res) => {
                        if (err) {
                          console.log("expect ==>", err);
                          ...
                          describe('Reset Password page', function () {
                            it('status ', function (done) {
                              chai.request(server).post('/reset').send(data.resetPassword).end((err, res) => {
                                if (err) {
                                  console.log("expect ==>", err);
                                  err.should.have.status(400); //The request had bad syntax or was inherently impossible to be satisfied.
                                } else {
